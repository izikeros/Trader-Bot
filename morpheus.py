...
class Morpheus:
    ...
    def get_historical_prices(
        ...
        while (int(end_msecs) - int(most_recent)) >= interval_msecs:
            hist_data = [
                self.__binance.get_kline_data(
                    base_asset, quote_asset, period_interval, most_recent, end_msecs
                )
            ]
            for d in hist_data:
                self.__database.insert_market_data(base_asset, quote_asset, d)
            most_recent = self.__database.get_most_recent_pair_period_close(
                base_asset, quote_asset, period_interval
            )
            count_api_calls += 1
        return
    ...
    def get_historical_data(
        self, start_date=None, end_date=datetime.datetime.now(), live_feed=False
    ):
        for base_asset, quote_asset in self.TRADING_PAIRS:
            ...
            end_msecs = (
                api_start_track
                if end_date is None
                else int(end_date.timestamp() * 1000)
            )
            count_api_calls = 0  # LIMIT IS 20 per second
            while int(most_recent) < (int(end_msecs)):
                hist_data = [
                    self.__binance.get_kline_data(
                        base_asset, quote_asset, interv, most_recent, end_msecs
                    )
                ...
                print(
                    ...
                    + quote_asset
                    + " write: "
                    + str(reduce(lambda s, y: s and y, write_status))
                )
                now_msecs = int(datetime.datetime.now().timestamp() * 1000)
                if (
                    count_api_calls >= 1100 and (now_msecs - api_start_track) <= 60000
                ):  # 1minute check
                ...
                time_ = datetime.datetime.fromtimestamp(time_).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                end_time_ = end_msecs / 1000
                end_time_ = datetime.datetime.fromtimestamp(end_time_).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                print(time_)
                print(end_time_)
